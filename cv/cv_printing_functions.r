...
print_skill_bars <- function(cv, out_of = 5, bar_color = "#6394c6", bar_background = "#d9d9d9", glue_template = "default"){
...
#' @description Create linkedin url instead of full url text for better display
create_github_url <- function(github_url){
  return (glue::glue("<a href='https://{github_url}'>JS-Choi513</a>"))
}

#' @description Create linkedin url instead of full url text for better display
create_blog_url <- function(blog_url){
  return (glue::glue("<a href='https://{blog_url}'>Blog</a>"))
}
...
print_contact_info <- function(cv){
  ...
  cv$contact_info %<>% dplyr::mutate(contact =  ifelse(grepl("github", loc), create_github_url(contact), contact))
  cv$contact_info %<>% dplyr::mutate(contact =  ifelse(grepl("website", loc), create_blog_url(contact), contact))
